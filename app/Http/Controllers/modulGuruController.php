...
class modulGuruController extends Controller
{
    ...
    function tampilan_presensiAbsen(){
        ...
        if (session("hasLogin")){
            $total_kehadiran = master_absen_guru::where("status_kehadiran","h")->where("guru_id",session("id"))->whereMonth("tgl_masuk",Carbon::now()->translatedFormat("m"))->count();
            $total_lambat = master_absen_guru::where("status_kehadiran","h")->where("guru_id",session("id"))->whereMonth("tgl_masuk",Carbon::now()->translatedFormat("m"))->where("terlambat_menit","!=",0)->count();
