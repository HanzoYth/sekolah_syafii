...
class MasterAbsenController extends Controller
{
    ...
    function keluarAbsenGuru(){
        $data = master_absen_guru::where("tgl_masuk",Carbon::now()->translatedFormat("Y-m-d"))->where("waktu_masuk","!=",Carbon::parse("00:00:00")->translatedFormat("H:i:s"))->where("waktu_keluar",Carbon::parse("00:00:00")->translatedFormat("H:i:s"))->whereNotIn("status_kehadiran",["a","s","i"])->where("guru_id",session("id"))->first();
        dd([
            "tes" => $data
        ]);
